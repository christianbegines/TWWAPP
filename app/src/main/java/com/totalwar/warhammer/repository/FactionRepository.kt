...
class FactionRepository(
    ...
    fun getAllFactions() {
        coroutineScope.launch(Dispatchers.IO) {
            (apolloClient.query(FactionsQuery()).execute().data?.tww as FactionsQuery.Tww).let {
                it.factions?.let { factions ->
                    allFactions.postValue(factions)
                }
            }
        }
    }
    ...
    fun getVersion() {
        coroutineScope.launch(Dispatchers.IO) {
            (apolloClient.query(GameVersionsQuery()).execute().data?.versions as List<GameVersionsQuery.Version>).let {
